...
export function ShopProvider({ children }) {
  ...
  const [wishlist, setWishlist] = useState(() => readStorage('shahan-wishlist', []));
  const [user, setUserState] = useState(() => readStorage('shahan-user', null));
  const [orders, setOrders] = useState(() => readStorage('shahan-orders', []));
  const [addresses, setAddresses] = useState(() => readStorage('shahan-addresses', []));
  const [theme, setTheme] = useState(() => readStorage('shahan-theme', 'light'));
  ...
  const addToCart = (product) => {
    ...
    setToast({ type: 'success', message: `«${product.title}» به سبد خرید اضافه شد.` });
  };

  const removeFromCart = (id) => updateCart((prev) => prev.filter((i) => i.id !== id));
  ...
  const addAddress = (address) => {
    ...
    updateAddresses((prev) => {
      ...
      return prev.length === 0 ? next.map((a) => ({ ...a, isDefault: a.id === withId.id })) : next;
    });
    return withId;
  };

  const updateAddress = (id, patch) =>
    updateAddresses((prev) => prev.map((a) => (a.id === id ? { ...a, ...patch } : a)));
  ...
  const setDefaultAddress = (id) =>
    updateAddresses((prev) => prev.map((a) => ({ ...a, isDefault: a.id === id })));
